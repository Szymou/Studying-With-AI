...
const questions = [
    ...
    ['Java基础', '枚举', 'Java枚举的特点？', '枚举是特殊的类，默认继承Enum，可定义字段、方法、构造函数。单例模式枚举实现最安全。'],
    ['Java基础', '序列化', 'Java序列化和反序列化？', '实现Serializable接口，transient修饰变量不序列化。serialVersionUID用于版本兼容。'],
    ['Java基础', '复制', '浅拷贝和深拷贝的区别？', '浅拷贝只复制引用，深拷贝复制整个对象结构。通过clone()方法或序列化实现深拷贝。'],
    ['Java基础', 'switch', 'switch支持哪些类型？', 'byte、short、int、char、String（Java7+）、枚举。不支持long、float、double。'],
    ['Java基础', '类加载', '类加载器分类？', 'Bootstrap（核心类）、Extension（扩展类）、Application（classpath）、Custom（自定义）。'],
    ...
    ['并发', '线程', 'Callable和Runnable的区别？', 'Callable可返回结果并抛出异常，Runnable无返回值。FutureTask包装Callable。'],
    ['并发', '原子类', 'AtomicInteger的实现？', '基于CAS循环，保证原子操作。常用方法：incrementAndGet、compareAndSet。'],
    ['并发', '锁', '自旋锁是什么？', '线程循环等待锁释放，避免上下文切换。适用于锁持有时间短、竞争不激烈的场景。'],
...
// 总计：20+15+18+15+10+8+6+10+6+5+6 = 119题
