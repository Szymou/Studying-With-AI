...
router.post('/ask', auth_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        // 定期保存进度的函数
        const saveProgress = async (content) => {
            if (questionId && content.length > 0) {
                try {
                    await db_1.default.run('DELETE FROM ai_answer_cache WHERE user_id = ? AND question_id = ?', [req.user.userId, questionId]);
                    await db_1.default.run('INSERT INTO ai_answer_cache (user_id, question_id, answer) VALUES (?, ?, ?)', [req.user.userId, questionId, content]);
                }
                catch (e) {
        ...
        response.data.on('end', async () => {
            ...
            if (fullContent) {
                ...
                if (questionId) {
                    try {
                        await db_1.default.run('DELETE FROM ai_answer_cache WHERE user_id = ? AND question_id = ?', [req.user.userId, questionId]);
                        await db_1.default.run('INSERT INTO ai_answer_cache (user_id, question_id, answer) VALUES (?, ?, ?)', [req.user.userId, questionId, finalAnswer]);
                    }
                    catch (e) {
                ...
                // 流式失败 → 非流式重试
                try {
                    const fbRes = await axios_1.default.post(AI_BASE_URL + '/chat/completions', { model: AI_MODEL, messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: userPrompt }], temperature: 0.7, stream: false }, { headers: { 'Authorization': 'Bearer ' + AI_API_KEY, 'Content-Type': 'application/json' }, timeout: 15000 });
        ...
        response.data.on('error', async () => {
            ...
            if (fullContent.length > 0 && questionId) {
                try {
                    await db_1.default.run('DELETE FROM ai_answer_cache WHERE user_id = ? AND question_id = ?', [req.user.userId, questionId]);
                    await db_1.default.run('INSERT INTO ai_answer_cache (user_id, question_id, answer) VALUES (?, ?, ?)', [req.user.userId, questionId, fullContent]);
                }
                catch (e) {
...
router.post('/chat/:id/message/stream', async (req, res) => {
    try {
        const { message } = req.body;
        if (!message)
            return res.status(400).json({ error: '消息不能为空' });
        ...
        const saveProgress = async () => {
            if (fullReply.length > 0) {
                ...
                await db_1.default.run('UPDATE ai_conversations SET messages = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [JSON.stringify(tempMessages), req.params.id]);
            }
        };
        response.data.on('data', (chunk) => {
            buffer += chunk.toString();
...
router.post('/generate', async (req, res) => {
    try {
        ...
        const domainMap = { java: 'Java', go: 'Go', python: 'Python', frontend: '前端', database: '数据库', devops: 'DevOps' };
...
router.post('/generate-domain', async (req, res) => {
    try {
        ...
        if (icon)
            domainInfo.icon = icon;
        // 检查领域是否已存在
        const existingDomain = await db_1.default.get('SELECT * FROM tech_domains WHERE code = ?', [domainInfo.code]);
        if (existingDomain) {
            return res.json({ success: false, message: '该领域已存在', domain: existingDomain });
        }
        // 2. 保存新领域
        await db_1.default.run('INSERT INTO tech_domains (code, name, icon, description, user_id) VALUES (?, ?, ?, ?, ?)', [domainInfo.code, domainInfo.name, domainInfo.icon, domainInfo.description, req.user.userId]);
        // 3. 生成题目
        let savedCount = 0;
        try {
            ...
            for (const q of questions) {
                if (!q.question || !q.answer)
                    continue;
                try {
                    await db_1.default.run('INSERT INTO questions (category, subcategory, question, answer, difficulty, tags, tech_domain) VALUES (?, ?, ?, ?, ?, ?, ?)', [q.category || '基础', q.subcategory || '', q.question, q.answer, q.difficulty || 'medium', q.tags || '', domainInfo.code]);
                    savedCount++;
                }
                catch (e) {
...
// AI answer cache
router.get('/cache/:qid', async (req, res) => {
    try {
        const row = await db_1.default.get('SELECT answer FROM ai_answer_cache WHERE user_id = ? AND question_id = ?', [req.user.userId, req.params.qid]);
        if (row)
...
router.post('/cache', async (req, res) => {
    try {
        ...
        await db_1.default.run('DELETE FROM ai_answer_cache WHERE user_id = ? AND question_id = ?', [req.user.userId, questionId]);
        await db_1.default.run('INSERT INTO ai_answer_cache (user_id, question_id, answer) VALUES (?, ?, ?)', [req.user.userId, questionId, answer]);
