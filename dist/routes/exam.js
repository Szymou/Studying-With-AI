...
router.post('/submit', async (req, res) => {
    try {
        ...
        const record = await db_1.default.get(`
    ...
    `, [userId]);
        ...
        for (let i = 0; i < questions.length; i++) {
            ...
            const correctAnswer = await db_1.default.get('SELECT answer FROM questions WHERE id = ?', [q.id]);
...
router.get('/history', async (req, res) => {
    try {
        const records = await db_1.default.all(`
