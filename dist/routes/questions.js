...
router.post('/:id/answer', async (req, res) => {
    try {
        ...
        const question = await db_1.default.get('SELECT answer FROM questions WHERE id = ?', [questionId]);
        if (!question) {
            return res.status(404).json({ error: '题目不存在' });
        }
        const isCorrect = userAnswer && userAnswer.trim().toLowerCase() === question.answer.trim().toLowerCase();
        await db_1.default.run('INSERT INTO user_progress (user_id, question_id, is_correct) VALUES (?, ?, ?)', [userId, questionId, isCorrect]);
...
router.get('/progress/stats', async (req, res) => {
    try {
        const userId = req.user.userId;
        const stats = await db_1.default.get(`
      SELECT 
