...
router.get('/:id', async (req, res) => {
    try {
        const userId = req.user.userId;
        const q = await db_1.default.get('SELECT * FROM custom_questions WHERE id = ? AND user_id = ?', [req.params.id, userId]);
        if (!q)
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        if (!existing)
            return res.status(404).json({ error: '题目不存在' });
        await db_1.default.run('DELETE FROM custom_questions WHERE id = ? AND user_id = ?', [req.params.id, userId]);
