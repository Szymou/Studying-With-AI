...
const saveConfig = (newToken) => {
    token = newToken;
    const configPath = CONFIG_PATH.replace(/^~/, process.env.HOME || '~');
    fs_1.default.writeFileSync(configPath, JSON.stringify({ token, serverUrl }, null, 2));
};
const api = () => {
    if (!token) {
...
program
    ...
    if (action === '查看收藏列表') {
        ...
        const { id, sourceType } = await inquirer_1.default.prompt([
...
program
    ...
    if (action === '查看列表') {
        ...
        const { question, answer, category, subcategory, tags } = await inquirer_1.default.prompt([
        ...
        const { id, question, answer, category } = await inquirer_1.default.prompt([
            { type: 'input', name: 'id', message: '题目ID:' },
...
program
    ...
    try {
        ...
        while (continueChat) {
            const { followUp } = await inquirer_1.default.prompt([
...
program
    ...
    try {
        ...
        const topic = options.topic || (await inquirer_1.default.prompt([
            { type: 'input', name: 'topic', message: '输入主题（如：ConcurrentHashMap、Spring事务、JVM垃圾回收）:' }
        ])).topic;
        const count = options.count || (await inquirer_1.default.prompt([
        ...
        if (res.data.parsed) {
            ...
            for (let i = 0; i < res.data.questions.length; i++) {
                ...
                const { show } = await inquirer_1.default.prompt([
