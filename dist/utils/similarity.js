...
exports.isDuplicateQuestion = isDuplicateQuestion;
/**
 * 将字符串拆分为字符二元组 (bigram) 集合
 * 中英文混合场景下，字符级 bigram 比分词更鲁棒
 */
function toCharBigrams(s) {
    const cleaned = s
        .replace(/[\s,，。！？、；：""''（）()【】《》\[\]{}#@!?.;:<>\/\\|`~…—\-=+*&^%$￥…·]+/g, '')
        .toLowerCase();
 ...
 */
async function isDuplicateQuestion(question, techDomain, db, threshold = 0.8) {
    ...
    const existingQuestions = [
        ...systemQuestions.map((r) => r.question),
        ...customQuestions.map((r) => r.question),
    ];
